...
async function init(){
  try{
    ...
    window.gameSnapshot=()=>({view,phase:round.phase,player:{x:round.player.x,z:round.player.z,angle:round.player.angle},elapsed:round.elapsed,maze:maze.id,hunter:round.leakyPad?null:{x:round.hunter.x,z:round.hunter.z,stunned:round.hunter.stunned},miniatures:round.miniatures.map(h=>({id:h.id,state:h.state,x:h.x,y:h.y||0,z:h.z,scale:h.scale,speed:h.hunterSpeed})),music:audio.snapshot(),cats:round.cats.map(c=>({state:c.state,x:c.x,z:c.z,timer:c.timer,pickupCooldown:c.pickupCooldown})),food:round.food,foodCooldown:round.foodCooldown,captureTime:round.capture?.elapsed,remaining:round.remaining,duration:round.duration,power:padPower(round),heldCat:round.heldCat,scores:{...scores},routeLength:maze.route.length,drawCalls:world.renderer.info.render.calls});
...
async function selectLevel(id){
  ...
  try{
    ...
    $('preview-panel').querySelector('p').innerHTML=round.leakyPad?'Start at the pad’s lower-right opening. Follow the red arrows into the clothes.<br>Power-ups at 1:33–1:52 and 3:06–3:24. Find the pink exit light.':'You start at the green marker. Stickman starts at the peach exit.<br>Find a kitty if things get a little too close.';

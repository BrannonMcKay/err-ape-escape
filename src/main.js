...
import {tickMiniatures} from './leaky-pad.js';
...
function hud(){
  ...
  if(round.leakyPad){const attached=round.miniatures.filter(m=>m.state==='attached').length,next=round.elapsed<30?30:30+8*round.miniatures.length;
    ...
    $('biome').textContent=round.elapsed>=89?'Gloaming · follow the lamps':round.elapsed>=60?'Red ocean · fading daylight':round.elapsed>=30?'Red lights · the pad is leaking':'Teal ocean · sunset begins';
  }
...
async function init(){
  try{
    ...
    window.gameSnapshot=()=>({view,phase:round.phase,player:{x:round.player.x,z:round.player.z,angle:round.player.angle},elapsed:round.elapsed,maze:maze.id,hunter:round.leakyPad?null:{x:round.hunter.x,z:round.hunter.z,stunned:round.hunter.stunned},miniatures:round.miniatures.map(h=>({id:h.id,state:h.state,x:h.x,y:h.y||0,z:h.z,scale:h.scale,speed:h.hunterSpeed})),music:audio.snapshot(),cats:round.cats.map(c=>({state:c.state,x:c.x,z:c.z,timer:c.timer,pickupCooldown:c.pickupCooldown})),food:round.food,foodCooldown:round.foodCooldown,captureTime:round.capture?.elapsed,remaining:round.remaining,heldCat:round.heldCat,scores:{...scores},routeLength:maze.route.length,drawCalls:world.renderer.info.render.calls});
...
async function selectLevel(id){
  ...
  try{
    ...
    $('preview-panel').querySelector('p').innerHTML=round.leakyPad?'Follow the red arrows out of the pad. Find the pink light at the lower-right exit.<br>Tiny Stickmen begin at 0:30.':'You start at the green marker. Stickman starts at the peach exit.<br>Find a kitty if things get a little too close.';

...
export function resultScreen(result,round,scores){
  ...
  return `<div class="result-layout ${victory?'with-portrait':''}">${victory?`<figure class="victory-portrait"><img src="${victoryPortrait}" alt="Illustration of Christel smiling cheek-to-cheek with her fluffy tabby cat in the woods"><figcaption>A PURR-FECT ESCAPE</figcaption></figure>`:''}<section class="result-copy"><span class="modal-eyebrow">${text.eyebrow}</span><h2 class="modal-title">${text.title}</h2><div class="score-result" style="${result==='caught'?'color:#e6aaa9':''}">${text.points}</div><p class="modal-lede">${text.note}</p><p class="result-note">${Math.round(round.elapsed)} seconds in the maze · ${round.settings.name} chase</p><div class="session-score"><span>YOUR SCORE <b>${scores.player}</b></span><span>STICKMAN <b>${scores.stickman}</b></span></div><div class="modal-buttons"><button id="again" class="primary">Run it back <b>↗</b></button><button id="result-home" class="secondary">Back to mazes</button></div>${scores.player>=5?'<p class="fineprint">Your screening-room unlocks are ready in the top menu.</p>':''}</section></div>`;
}
